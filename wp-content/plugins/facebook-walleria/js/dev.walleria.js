jQuery(document).ready(function($){
 ...
 var fbroot=$('#fb-root'),
     userID='',
     token='',
     likebutton=$('<a class="fp-buttonLink  clearfix"><i></i><span class="fp-likePhoto">Like</span></a>'),
     alreadylike=$('<span class="fp-youLikeThis">'+intl.youlike+'</span>'),
     tagbutton=$('<a class="fp-buttonLink " ><i></i><span class="fp-tagPhoto">Tag Photo</span></a>'),
     sharebutton=$('<a class="fp-buttonLink  clearfix"><i></i><span class="fp-sharePhoto">Share</span></a>'),
...
$.post(settings.ajaxurl,{'action':'getevents','id':id, 'limit':limit,'unix':unix}, function(data) {
   ...
   obj.closest('.fp-EventsContainer').html(data)
    ...
    //change the next url
    var nexttime=fwpg_sanitize_nextpage_query(eventlist['paging']['next']) ;
    var previoustime=fwpg_sanitize_prevpage_query(eventlist['paging']['previous']) ;
    obj.closest('.fp-RightFloat').find('.fp-PreviousEventsPage').attr('data-href',previoustime)
    obj.closest('.fp-RightFloat').find('.fp-NextEventsPage').attr('data-href',nexttime)
    obj.closest('.fp-EventsContainer').find('.fp-EventsInnerWrapper').html(html);
//return false;
//close test for array
